...
void drawCurrentMood() {
    ...
    if (show_stats) {
        ...
        tft.printf("Session Pwns: %lu", static_cast<unsigned long>(session_captured));
        ...
        tft.setTextSize(1);
        tft.setCursor(10, tftHeight - 20);
        tft.printf("Lvl: %d | Pwned: %lu", (total_pwned / 5) + 1, static_cast<unsigned long>(session_captured));
    }
}
...
void setMood(String face, String text) {
    ...
    current_text = text;
    Serial.printf(
        "[Brucegotchi] [DISPLAY] %s | %s | Pwns: %lu\n", face.c_str(), text.c_str(),
        static_cast<unsigned long>(session_captured)
    );
    drawCurrentMood();
}
